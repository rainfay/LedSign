...
/*
 *     This program time is a client program, that sends the command to display the time to the server.
...
*/
 if (3 != argc) {
    ...
    freeaddrinfo(servinfo); // all done with this structure
      ...
      temp=sprintf(stringtemp,"AA%c b%c%c%c%c %c %c",smf,cc,green,fontmode,normallargefont,timeLED,end); //see DAE code for commands
        temp=write(sockfd,stringtemp,temp); // normal shove out the door code
	endloop(sockfd);
        read(sockfd, buffer, sizeof(buffer)); // read data from the server
        printf("%s \n",buffer); //display on consle
